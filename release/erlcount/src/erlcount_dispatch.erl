...
handle_event(
    ...
    State,
    Data = #data{regex=Regexes, refs=Refs}
) ->
    {Regex, OldCount} = lists:keyfind(Regex, 1, Regexes),
    NewRegexes = lists:keyreplace(Regex, 1, Regexes, {Regex, OldCount + Count}),
    NewData = Data#data{regex=NewRegexes, refs=Refs--[Ref]},
    case State of
        dispatching ->
...
%%% FSM STATES
dispatching({continue, File, Continuation}, Data = #data{regex=Regexes, refs=Refs}) ->
...
listening(done, #data{regex=Regexes, refs=[]}) ->  % all received!
